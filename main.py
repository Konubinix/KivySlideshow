...
class MyCarousel(Carousel):
    ...
    def start_automatic(self):
        ...
        Clock.schedule_interval(self.load_next, global_config.getint("config", "slide_time_interval"))
...
"""
    ...
    def renew_sources(self, dt=None):
        ...
        # images = [
        # "Aylapomme/2014/0405-Maison/140405_132744_GT-I9300.jpg",
        # "Aylapomme/2014/0405-Maison/140405_132748_GT-I9300.jpg",
        # "Aylapomme/2014/0405-Maison/140405_132759_GT-I9300.jpg",
        # "Aylapomme/2014/0405-Maison/140405_132804_GT-I9300.jpg",
        # "Aylapomme/2014/0405-Maison/140405_132811_GT-I9300.jpg",
        # "Aylapomme/2014/0405-Maison/140405_132814_GT-I9300.jpg",
        # "Aylapomme/2014/0405-Maison/140405_132816_GT-I9300.jpg",
        # ]
        self.sources = images
    ...
    def renew_source_clock(self):
        ...
        Clock.schedule_interval(self.renew_sources, global_config.getint("config", 'change_picture_interval'))
    ...
    def callback_current_delete(self, *args):
        args[0].parent.dismiss()
        res = server.set_image_delete(
            self.sources[self.current_index % len(self.sources)]
        )
        self.popup_result(res)
        if res == 0:
            del self.sources[self.current_index % len(self.sources)]
            self.update_images(True)

    def callback_current_set_private(self, *args):
        args[0].parent.dismiss()
        res = server.set_image_private(
            self.sources[self.current_index % len(self.sources)]
        )
        self.popup_result(res)
        if res == 0:
...
class CarouselApp(App):
    def build_config(self, config):
        config.setdefaults(
            'config', {
                'slide_time_interval': 60,
                'change_picture_interval': 60 * 60 * 24,
            }
        )
    ...
    def get_application_config(self):
        if plyer.utils.platform == "linux":
            return super(CarouselApp, self).get_application_config(
                '~/.%(appname)s.ini')
        else:
            return super(CarouselApp, self).get_application_config()
    ...
    def on_config_change(self, config, section, key, value):
        if config is not self.config:
            return
        token = (section, key)
        if token == ('config', 'slide_time_interval'):
            carousel.on_slide_time_interval()
        elif token == ('config', 'change_picture_interval'):
            carousel.on_change_picture_interval()
